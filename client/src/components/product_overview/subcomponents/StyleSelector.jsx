...
const StyleSelector = ({ product, styles, currentStyle, setCurrentStyle, mainImgSize }) => {
    ...
    return (
        <div style={{ background: 'none', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', alignContent: 'center', flexBasis: '43%', padding: '25px', display: mainImgSize ? 'none' : 'block' }}>
                ...
                {styles[0] ?
                    styles.map((style, index) => {
                        return <div style={{ width: '70px', height: '70px', padding: '.75em' }}>
                            ...
                            <img src={style.photos[0].thumbnail_url} id={index} alt="" onClick={(e) => { setCurrentStyle(e.target.id); setChecked(index) }} style={{ zIndex: '-100', border: '2px solid black', width: '100%', height: '100%', objectFit: 'cover', marginBottom: '1em', borderRadius: '50px' }} />
                        </div>
                    })
                    : <img></img>
                }
            ...
            {/* Options */}
            <div>
                <select name="size" id="size" onChange={(e) => { setCurrentSize(e.target.value) }} style={{ fontSize: '1.5em', paddingLeft: '50px', paddingRight: '70px', paddingTop: '20px', paddingBottom: '20px', marginLeft: '.2em', marginRight: '.25em' }}>
            ...
            <div>
                <button onClick={() => { if (currentQuantity == null) { sizeSelect.focus() } }} style={{ backgroundColor: 'darkgrey', fontSize: '1.5em', paddingLeft: '50px', paddingRight: '70px', paddingTop: '20px', paddingBottom: '20px', marginLeft: '.2em', marginRight: '.2em', marginTop: '1em' }}>
                    ADD TO CART
                </button>
